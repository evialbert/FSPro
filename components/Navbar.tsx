    ...
    <nav className="bg-[#003366] text-white sticky top-0 z-[100] shadow-lg font-sans">
          ...
          <div className="relative group">
            ...
            <div className="absolute top-full left-0 w-72 bg-white text-slate-800 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-2 group-hover:translate-y-0 p-2 border border-slate-100">
              {finantariSubmenu.map((sub) => (
                <Link key={sub.href} href={sub.href} className="flex items-center space-x-3 p-3 hover:bg-blue-50 rounded-lg transition-colors group/item">
                  <div className="text-blue-600">{sub.icon}</div>
                  <span className="group-hover/item:text-blue-700">{sub.name}</span>
                </Link>
              ))}
            </div>
          </div>

          {/* Dropdown Alte Servicii Desktop */}
          <div className="relative group">
            <button className="flex items-center space-x-1 hover:text-blue-400 transition-colors py-8 outline-none">
      ...
      >
        <div className="flex flex-col p-6 space-y-3">
          ...
          {/* Submeniu Mobil Finantari */}
          <div className="flex flex-col">
            <button 
              onClick={() => setIsFinantariOpen(!isFinantariOpen)}
              className="flex justify-between items-center py-2 border-b border-blue-900/50 text-blue-300 outline-none"
            >
              Finanțări nerambursabile <ChevronDown size={20} className={`transition-transform duration-300 ${isFinantariOpen ? "rotate-180" : ""}`} />
            </button>
            {isFinantariOpen && (
              <div className="bg-[#001f3d] rounded-lg mt-2 p-2 flex flex-col space-y-2 animate-in fade-in slide-in-from-top-2">
                {finantariSubmenu.map((sub) => (
          ...
          {/* Submeniu Mobil Alte Servicii */}
          <div className="flex flex-col">
            <button 
              onClick={() => setIsServiciiOpen(!isServiciiOpen)}
              className="flex justify-between items-center py-2 border-b border-blue-900/50 text-blue-300 outline